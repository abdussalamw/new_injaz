                        ...
                        <table class="table table-sm table-hover" id="employeeStatsTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-success" title="<?= \App\Core\RoleHelper::getRoleCompletionDescription($emp['role']) ?>"><?= $emp['completed_tasks'] ?></span>
                                    </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('radarComparisonChart')) {
        ...
        const completedTasksData = [<?php echo implode(',', array_map(function($emp) { return $emp['completed_tasks']; }, $employee_stats)); ?>];
        const revenueTasksData = [<?php echo implode(',', array_map(function($emp) { return $emp['total_revenue'] ?? 0; }, $employee_stats)); ?>];
        const maxCompletedTasks = Math.max(...completedTasksData);
        const maxRevenue = Math.max(...revenueTasksData);
